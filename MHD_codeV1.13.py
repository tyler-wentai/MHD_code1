...
nstep, nstep_end = 71401, 85001
timeSIM     = 1080.06           # simulated time
shearMAX    = 10.5506           # simulated shear
...
tACC        = 50.           # time required for shear to accelerate (deaccelerate) to max speed vzMAX (zero)
tDEC        = 1030.         # time at which deacceleration of the shearing motion begins
...
### ------------- MAIN FUNCTION ------------- ###
def main():
    global nstep, timeSIM, shearMAX, resiLOG
...
#                             /(dycCOEF[j,0,0])                
    ...
    if (restartLOG==False):
        ...
        xFILE_IN    = open("V1.12_n71401","rb")
        xHELP       = np.load(xFILE_IN)
        ...
        for i in range(mxp):
            for j in range(myp):
                ...
                cur[i,j,0] = xHELP[i,j,9]
                cur[i,j,1] = xHELP[i,j,10]
                cur[i,j,2] = xHELP[i,j,11]
...
def stepon(dt, alpha, timeSIM, cfl):
    ...
    # (5): Add time advance terms to psi and Bz if resistivity is turned on:
    if (resiLOG==True):
        xRES = np.zeros((mxp,myp,2),   dtype=np.double)
        for i in range(mxp):
            for j in range(myp):
                xRES[i,j,0] = x[i,j,4]
                xRES[i,j,1] = x[i,j,5]
    
        calc_innRESST(dt,xRES)
        calc_outRESST(dt,xRES)
            
    # (6): Update the advanced/updated values of the Bx,By,J:
    calc_bxby(x[:,:,4], bx, by)
...
def calc_outPRDCT(dt, alpha, xOLD, xPRDCT, timeSIM):
    ...
    for i in range(0,mxp):
        ...
        elif (timeSIM>=tDEC and timeSIM<(tDEC+tACC)):
            velSHEAR = vzMAX*xx[i]*np.exp(0.5*(1.0 - xx[i]**2))
            xPRDCT[i,0,3] = xOLD[i,0,3] - alpha*dt*(velSHEAR/tACC)
        elif (timeSIM>(tDEC+tACC)):
            xPRDCT[i,0,3] = +0.0
        else:
...
def calc_correctRIGHT(s, vRIGHT,    v_old,       \
    ...
    if (s==1 or s==2):
        ...
        elif ((timeSIM+dt)>=tDEC and (timeSIM+dt)<(tDEC+tACC)):
            for i in range(mxp):
                vRIGHT[i,0] = (1. - ((timeSIM+dt-tDEC)/tACC))*vzMAX*xx[i]*np.exp(0.5*(1.0-xx[i]**2))
        elif ((timeSIM+dt)>=(tDEC+tACC)):
            for i in range(mxp):
                vRIGHT[i,0] = 0.0
        else:
            for i in range(mxp):
...
#
def calc_innRESST(dt, xRES_IN):
    # Calculate the resistive terms for psi and Bz and update the x array accordingly for all inner
    # grid points.
    ...
    for s in range(2):
        for i in range(1,mxp-1):
            for j in range(1,myp-1):
                ...
                termHELP =   dfdxc('2',i,xRES_IN[i+1,j,s],xRES_IN[i+0,j,s],xRES_IN[i-1,j,s])     \
                           + dfdyc('2',j,xRES_IN[i,j+1,s],xRES_IN[i,j+0,s],xRES_IN[i,j-1,s])
                           
                x[i,j,s+4] += dt*resiCOEF*termHELP
 

#
def calc_outRESST(dt, xRES):
    # Calculate the resistive terms for psi and Bz and update the x array accordingly for all
    ...
    for s in range(2):
        for j in range(1,myp-1):
            ...
            termHELP =   dfdxc('2',0,xRES[1,j,s],xRES[0,j,s],xRES[1,j,s])     \
                       + dfdyc('2',j,xRES[0,j+1,s],xRES[0,j+0,s],xRES[0,j-1,s])
    
            x[0,j,s+4] += dt*resiCOEF*termHELP
    ...
    for s in range(2):
        for i in range(0,mxp-1):
            ...
            if (i==0):
                termHELP =   dfdxp('2',i,xRES[i+2,myp-1,s],xRES[i+1,myp-1,s],xRES[i+0,myp-1,s])     \
                           + dfdym('2',myp-1,xRES[i,myp-1,s],xRES[i,myp-2,s],xRES[i,myp-3,s])
            else:
                termHELP =   dfdxc('2',i,xRES[i+1,myp-1,s],xRES[i+0,myp-1,s],xRES[i-1,myp-1,s])     \
                           + dfdym('2',myp-1,xRES[i,myp-1,s],xRES[i,myp-2,s],xRES[i,myp-3,s])
                           
            x[i,myp-1,s+4] += dt*resiCOEF*termHELP
    ...
    for s in range(2):
        for j in range(1,myp):
            ...
            if (j==myp-1):
                termHELP =   dfdxm('2',mxp-1,xRES[mxp-1,j,s],xRES[mxp-2,j,s],xRES[mxp-3,j,s])     \
                           + dfdym('2',j,xRES[mxp-1,myp-1,s],xRES[mxp-1,myp-2,s],xRES[mxp-1,myp-3,s])
            else:
                termHELP =   dfdxm('2',mxp-1,xRES[mxp-1,j,s],xRES[mxp-2,j,s],xRES[mxp-3,j,s])     \
                           + dfdyc('2',j,xRES[mxp-1,j+1,s],xRES[mxp-1,j+0,s],xRES[mxp-1,j-1,s])
                           
            x[mxp-1,j,s+4] += dt*resiCOEF*termHELP
    
    # (4): Bottom boundary ----------------------------
    for i in range(0,mxp):
        # no need to update psi, it is constant at the bottom boundary
        s = 1
        
        if (i==0):
            termHELP =   dfdxc('2',i,xRES[i+1,0,s],xRES[i+0,0,s],xRES[i+1,0,s])         \
                       + dfdyp('2',0,xRES[i,2,s],xRES[i,1,s],xRES[i,0,s])
        elif (i==mxp-1):
            termHELP =   dfdxm('2',i,xRES[mxp-1,0,s],xRES[mxp-2,0,s],xRES[mxp-3,0,s])   \
                       + dfdyp('2',0,xRES[i,2,s],xRES[i,1,s],xRES[i,0,s])
        else:
            termHELP =   dfdxc('2',i,xRES[i+1,0,s],xRES[i+0,0,s],xRES[i-1,0,s])         \
                       + dfdyp('2',0,xRES[i,2,s],xRES[i,1,s],xRES[i,0,s])
        
        x[i,0,s+4] += dt*resiCOEF*termHELP
