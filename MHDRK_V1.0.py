...
# INITIALIZE --------
mxp = int(143)  # total number of points in x
myp = int(143)  # total number of points in y
...
#    
def calc_cur(bz_IN, bx_IN, by_IN):
    ...
    for i in range(1,mxp-1):
        for j in range(1,myp-1):
            cur[i,j,2] =   dfdxc('1',i,dBy[i+1,j],dBy[i+0,j],dBy[i-1,j]) \
                         - dfdyc('1',j,dBx[i,j+1],dBx[i,j+0],dBx[i,j-1]) 
                                  
        # jz left-side boundary:
    i = 0
    for j in range(1,myp):
        if (j==myp-1):
            cur[i,j,2] =   dfdxp('1',i,dBy[i+2,j],dBy[i+1,j],dBy[i+0,j]) \
                         - dfdym('1',j,dBx[i,j+0],dBx[i,j-1],dBx[i,j-2])
        else:
            cur[i,j,2] =   dfdxp('1',i,dBy[i+2,j],dBy[i+1,j],dBy[i+0,j]) \
                         - dfdyc('1',j,dBx[i,j+1],dBx[i,j+0],dBx[i,j-1])
    ...
    for i in range(1,mxp):
        if (i==mxp-1):
            cur[i,j,2] =   dfdxm('1',i,dBy[i+0,j],dBy[i-1,j],dBy[i-2,j]) \
                         - dfdym('1',j,dBx[i,j+0],dBx[i,j-1],dBx[i,j-2])
        else:
            cur[i,j,2] =   dfdxc('1',i,dBy[i+1,j],dBy[i+0,j],dBy[i-1,j]) \
                         - dfdym('1',j,dBx[i,j+0],dBx[i,j-1],dBx[i,j-2])
    ...
    j = 0
    for i in range(0,mxp):
        cur[i,j,2] = - dfdyp('1',j,dBx[i,j+2],dBx[i,j+1],dBx[i,j+0])
...
def diagnostic_plotter(cfl):
    ...
    plt.suptitle("RKV1.0b | vis1,vis2=1/100,OFF | shear=%.5f | cfl=%.3f" %(shearMAX,cfl), fontsize=15)
...
#    plt.contour(xx,yy,x[:,:,4].T, levels, colors='k', linewidths=0.6)                   # no color
    ...
    stringTEST = "RKV1.0b_n" + str(nstep)
